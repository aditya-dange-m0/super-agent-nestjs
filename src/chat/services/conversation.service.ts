  ...
  updateHistory(
    ...
    if (history.length > this.maxConversationHistory) {
      const removedCount = history.splice(
        0,
        history.length - this.maxConversationHistory,
      ).length;
      this.logger.log(
        `Trimmed history for key ${key}, removed ${removedCount} messages.`,
      );
    }

    this.conversationStore.set(key, history);
    this.logger.log(
      `Updated history for key ${key}, current length: ${history.length}`,
    );
  }
