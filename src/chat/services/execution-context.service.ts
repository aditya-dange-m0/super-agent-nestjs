...
export class ExecutionContextService {
  ...
  addStepResult(stepId: string, result: any): void {
    ...
    this.logger.log(
      `Added step ${stepId} result: ${this.truncateResult(result)}`,
    );
  }
  ...
  getStepResult(stepId: string): any {
    ...
    this.logger.log(
      `Retrieved step ${stepId} result: ${this.truncateResult(result)}`,
    );
    return result;
  }
  ...
  enrichParametersWithContext(parameters: any): any {
    ...
    for (const [key, value] of Object.entries(enriched)) {
      if (typeof value === 'string' && value.startsWith('$step_')) {
        ...
        if (stepResult !== undefined) {
          ...
          changed = true;
          this.logger.log(
            `Enriched parameter '${key}' with result from step ${stepId}.`,
          );
        } else {
          this.logger.warn(
            `Could not enrich parameter '${key}': Step ${stepId} result not found.`,
          );
        }
      }
    }
    ...
    if (changed) {
      this.logger.log(
        `Parameters enriched. Original: ${JSON.stringify(parameters)}, Enriched: ${JSON.stringify(enriched)}`,
      );
    }
